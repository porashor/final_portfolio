  ...
  return (
    <div>
      ...
      <form onSubmit={(e)=>placeOrder(e, price, productID , clientText, clientFiles )} className='space-y-5'>
        <textarea required type='textarea' value={clientText} onChange={(e) => onClientText(e.target.value)} className='border-1 border-slate-400 w-full h-[220px] md:h-[120px] p-1 rounded-2xl mt-2 outline-none focus:ring ring-slate-700' minLength={0} maxLength={1000} placeholder='write with in 1000 character '/>
        ...
        <button type='submit' className='w-full py-2 bg-light px-2 outline-none focus:ring'>{loading ? <IoReloadCircleOutline className='text-xl animate-spin w-fit mx-auto'/> : "order now"}</button>
      </form>
    </div>
  )
}
